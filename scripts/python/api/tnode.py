...
class Node:
    ...
    def enableV2Plugin(self, args=""):
        ssh_object = self.sshConnect(self.username, self.password)
	command = "docker plugin set " + os.environ.get("CONTIV_V2PLUGIN_NAME", "") + " iflist=eth2,eth3 plugin_name=" + os.environ["CONTIV_V2PLUGIN_NAME"] + " ctrl_ip=" + self.addr + " listen_url=" + self.addr + ":9999 " + args + " >> /tmp/netplugin.log 2>&1"
	self.runCmd(command)
