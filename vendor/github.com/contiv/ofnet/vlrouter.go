		...
		case protocol.Type_Request:
			...
			if endpoint == nil {
				...
				if vl.agent.isInternal(endpoint) || vl.agent.isInternalBgp(endpoint) {
					...
					if vl.agent.GetRouterInfo() != nil {
						intf, _ = net.InterfaceByName(vl.agent.GetRouterInfo().VlanIntf)
					} else {
					...
					if endpoint != nil {
						if vl.agent.isInternal(endpoint) || vl.agent.isInternalBgp(endpoint) {
							...
							vl.agent.incrStats("ArpReqUnknownEndpointType")
							return
						}
					} else {
...
func (vl *Vlrouter) AddUplink(portNo uint32, ifname string) error {
...
func (vl *Vlrouter) RemoveUplink(portNo uint32, ifname string) error {
	vl.myRouterMac = nil
	vl.uplinkOfp = 0
	return nil
}
