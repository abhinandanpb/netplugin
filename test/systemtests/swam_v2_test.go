...
func (s *systemtestSuite) NewSwarmV2Exec(n *node) *swarmV2 {
	w := new(swarmV2)
	w.node = n
	//w.env = s.basicInfo.swarmV2Env + " "
	return w
}

func (w *swarmV2) newContainer(node *node, containerID, name string) (*container, error) {
...
func (w *swarmV2) runContainer(spec containerSpec) (*container, error) {
...
func (w *swarmV2) checkPingFailure(c *container, ipaddr string) error {
...
func (w *swarmV2) checkPing(c *container, ipaddr string) error {
...
func (w *swarmV2) checkPing6Failure(c *container, ipaddr string) error {
...
func (w *swarmV2) checkPing6(c *container, ipaddr string) error {
...
func (w *swarmV2) getIPAddr(c *container, dev string) (string, error) {
...
func (w *swarmV2) getIPv6Addr(c *container, dev string) (string, error) {
...
func (w *swarmV2) getMACAddr(c *container, dev string) (string, error) {
...
func (w *swarmV2) exec(c *container, args string) (string, error) {
...
func (w *swarmV2) execBG(c *container, args string) (string, error) {
	return c.node.runCommand(fmt.Sprintf(w.env+"docker exec -d %s %s", c.containerID, args))
}

func (w *swarmV2) swarmV2Cmd(c *container, arg string) error {
...
func (w *swarmV2) start(c *container) error {
	logrus.Infof("Starting container %s on %s", c.containerID, c.node.Name())
	return w.swarmV2Cmd(c, "start")
}

func (w *swarmV2) stop(c *container) error {
	logrus.Infof("Stopping container %s on %s", c.containerID, c.node.Name())
	return w.swarmV2Cmd(c, "stop")
}

func (w *swarmV2) rm(c *container) error {
	logrus.Infof("Removing container %s on %s", c.containerID, c.node.Name())
	w.swarmV2Cmd(c, "kill -s 9")
	return w.swarmV2Cmd(c, "rm -f")
}

func (w *swarmV2) startListener(c *container, port int, protocol string) error {
	var protoStr string
	if protocol == "udp" {
		protoStr = "-u"
	}
...
func (w *swarmV2) startIperfServer(c *container) error {
	logrus.Infof("starting iperf server on: %s", c)
	_, err := w.execBG(c, fmt.Sprintf("iperf -s -u"))
	return err
}

func (w *swarmV2) startIperfClient(c *container, ip, limit string, isErr bool) error {
...
func (w *swarmV2) tcFilterShow(bw string) error {
...
func (w *swarmV2) checkConnection(c *container, ipaddr, protocol string, port int) error {
	var protoStr string
...
func (w *swarmV2) checkNoConnection(c *container, ipaddr, protocol string, port int) error {
...
func (w *swarmV2) cleanupDockerNetwork() error {
...
func (w *swarmV2) cleanupContainers() error {
...
func (w *swarmV2) startNetplugin(args string) error {
	logrus.Infof("Starting netplugin on %s", w.node.Name())
  id,err:= w.getPluginContainerID()
  if err != nil {
    logrus.Errorf("Unable to stop netplugin, err:",err)
    return err
  }
  cmd:= fmt.Sprintf("sudo docker-runc exec -t %s touch %s",id, netpluginRestartFile)

  logrus.Infof("Running Command %s",cmd)
  return w.node.tbnode.RunCommand(cmd)
}

func (w *swarmV2) stopNetplugin() error {
	logrus.Infof("Stopping netplugin on %s", w.node.Name())
  id,err:= w.getPluginContainerID()
  if err != nil{
      logrus.Errorf("Unable to stop netplugin,err:%s",err)
      return err
  }
  w.node.tbnode.RunCommand("rm -f "+netpluginRestartFile)
  cmd:= fmt.Sprintf("sudo docker-runc exec -t %s pkill netplugin",id)
  logrus.Infof("Running Command %s",cmd)
	return w.node.tbnode.RunCommand(cmd)
}

func (w *swarmV2) stopNetmaster() error {
	logrus.Infof("Stopping netmaster on %s", w.node.Name())
  id,err:= w.getPluginContainerID()
  if err != nil{
      logrus.Errorf("Unable to stop netmaster,err:",err)
      return err
  }
  w.node.tbnode.RunCommand("rm -f "+netmasterRestartFile)
  cmd:= fmt.Sprintf("sudo docker-runc exec -t %s pkill netmaster",id)
  logrus.Infof("Running Command %s",cmd)
  return w.node.tbnode.RunCommand(cmd)
}

func (w *swarmV2) startNetmaster() error {
	logrus.Infof("Starting netmaster on %s", w.node.Name())
  id,err:= w.getPluginContainerID()
  if err != nil {
    logrus.Errorf("Unable to start netmaster, err:",err)
    return err
  }
  cmd:= fmt.Sprintf("sudo docker-runc exec -t %s touch %s",id,netmasterRestartFile)
  return w.node.tbnode.RunCommand(cmd)
}
func (w *swarmV2) cleanupMaster() {
...
func (w *swarmV2) cleanupSlave() {
	...
	cmd:= "sudo docker-runc exec -t "+id+ "sh -c \"ovs-vsctl del-br contivVxlanBridge && ovs-vsctl del-br contivVlanBridge && ovs-vsctl del-br contivHostBridge"
	vNode.RunCommand(cmd)
	vNode.RunCommand("for p in `ifconfig  | grep vport | awk '{print $1}'`; do sudo ip link delete $p type veth; done")
}

func (w *swarmV2) runCommandUntilNoNetpluginError() error {
  id,err:= w.getPluginContainerID()
  if err != nil {
    logrus.Errorf("Unable to start netmaster, err:",err)
    return err
  }
  cmd:= fmt.Sprintf("sudo docker-runc exec -t %s pgrep netplugin",id)
  logrus.Infof("RUNNING COMAND:%s",cmd)
	return w.node.runCommandUntilNoError(cmd) 
}

func (w *swarmV2) runCommandUntilNoNetmasterError() error {
  id,err:= w.getPluginContainerID()
  if err != nil {
    logrus.Errorf("Unable to start netmaster, err:",err)
    return err
  }
  cmd:= fmt.Sprintf("sudo docker-runc exec -t %s pgrep netmaster",id)
  logrus.Infof("RUNNING COMAND:%s",cmd)
	return w.node.runCommandUntilNoError(cmd)
}

func (w *swarmV2) rotateNetmasterLog() error {
	return w.rotateLog("netmaster")
}

func (w *swarmV2) rotateNetpluginLog() error {
	return w.rotateLog("netplugin")
}

func (w *swarmV2) checkForNetpluginErrors() error {
	out, _ := w.node.tbnode.RunCommandWithOutput(`for i in /var/run/contiv/log/net*; do grep "panic\|fatal" $i; done`)
	if out != "" {
	...
	out, _ = w.node.tbnode.RunCommandWithOutput(`for i in /var/run/contiv/log/net*; do grep "error" $i; done`)
	if out != "" {
...
func (w *swarmV2) checkConnectionRetry(c *container, ipaddr, protocol string, port, delay, retries int) error {
	var protoStr string
	var err error
...
func (w *swarmV2) checkNoConnectionRetry(c *container, ipaddr, protocol string, port, delay, retries int) error {
	logrus.Infof("Expecting connection to fail from %v to %s on port %d", c, ipaddr, port)
...
func (w *swarmV2) checkPing6WithCount(c *container, ipaddr string, count int) error {
	logrus.Infof("Checking ping6 from %v to %s", c, ipaddr)
...
func (w *swarmV2) checkPingWithCount(c *container, ipaddr string, count int) error {
...
func (w *swarmV2) checkSchedulerNetworkCreated(nwName string, expectedOp bool) error {
...
func (w *swarmV2) checkSchedulerNetworkOnNodeCreated(nwNames []string, n *node) error {
...
func (w *swarmV2) verifyVTEPs(expVTEPS map[string]bool) (string, error) {
	var data interface{}
...
func (w *swarmV2) verifyEPs(epList []string) (string, error) {
...
func (w *swarmV2) reloadNode(n *node) error {
...
func (w *swarmV2) getMasterIP() (string, error) {
	return w.node.getIPAddr("eth1")
}

func (w *swarmV2) getPluginContainerID() (string,error) {

    cmd:= `docker plugin list | grep v2plugin | cut -d  " "  -f 1 `

		id,err:= w.node.tbnode.RunCommandWithOutput(cmd)
		if err != nil{
			return "",err
		}
		id = strings.TrimSpace(id)
    cmd = `sudo docker-runc list | grep ` + id + ` | cut -d " " -f 1`

    logrus.Infof("Running command %s on node : %s",cmd,w.node.Name() , w.node.tbnode.GetName())
    id,err = w.node.tbnode.RunCommandWithOutput(cmd)
    if err != nil{
      return "",err
    }
